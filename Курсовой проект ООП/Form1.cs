...
namespace Курсовой_проект_ООП
{
    public partial class MainForm : Form
    {
        public MainForm()
        {
            InitializeComponent();
            DoubleBuffered = true;
            ScoreCounter.Text = "СЧЕТ: 0";
            LivesCounter.Text = "ЖИЗНИ: 3";
    ...
    class Game
    {
        public event EventHandler OnBonusTriggered;
            ...
            foreach (Control x in gameArea.AreaView.Controls)
            {
                ...
                if (x is PictureBox && (string)x.Tag == "ScoreBonus")
                {
                    if (x.Top > gameArea.AreaView.Height)
                    {
                        gameArea.AreaView.Controls.Remove(x);
                        BonusIsLive = true;
                        OnBonusTriggered?.Invoke(this, EventArgs.Empty);
                    }
        ...
        private void GenerateBonus()
        {
            ...
            int random = randBonus.Next(1, 10);
